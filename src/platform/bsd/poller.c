...
/* NetBSD filter data type. */
#if PLATFORM == PLATFORM_NETBSD
typedef uint32_t filter_t;
typedef uint32_t flag_t;

/* Any other BSD filter data type. */
#else
...
typedef struct data {
    /**
     * @brief The BSD kqueue file descriptor.
     */
    int kq;

    /**
     * @brief The BSD kqueue items buffer for the received kqueue events.
     */
    struct kevent* items;
 ...
 * @return Kqueue flag.
 */
static inline flag_t code2flag(int code);
 ...
 * @return Poll code.
 */
static inline int filter2code(filter_t filter);
 ...
 * @return Poll code.
 */
static inline int flag2code(flag_t flag);
