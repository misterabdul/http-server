...
int kernel_sendfile(int socket, int file_fd, off_t file_size, off_t* sent) {
    ...
    for (ssize_t _ret; *sent < file_size;) {
        _ret = sendfile(socket, file_fd, sent, file_size);
        if (_ret > 0) {
            continue;
        }
...
int kernel_sendfile_secure(
     ...
     */
    for (ssize_t _ret; *sent < file_size;) {
        _ret = SSL_sendfile(ssl, file_fd, *sent, KTLS_CHUNK, 0);
        if (_ret > 0) {
            *sent += _ret;
            continue;
        }
...
int buffered_sendfile(
    ...
    for (ssize_t _ret; *sent < file_size;) {
        ...
        _ret = send(socket, buffer, _read, 0);
        if (_ret > 0) {
            *sent += _ret;
            continue;
        }
...
/**
 * @copydoc buffered_sendfile_secure
 */
int buffered_sendfile_secure(
    ...
    for (ssize_t _ret; *sent < file_size;) {
        ...
        _ret = SSL_write(ssl, buffer, _read);
        if (_ret > 0) {
            *sent += _ret;
            continue;
        }
